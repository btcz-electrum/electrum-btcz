...
FEERATE_FALLBACK_STATIC_FEE = 42000
...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrum-btcz directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(
                _('Electrum-BitcoinZ datadir does not exist. Was it deleted while running?') + '\n' +
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-btcz.conf into user_config[]."""
    if not path:
        return {}
