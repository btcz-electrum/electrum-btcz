...
The Kivy GUI is used with Electrum-BitcoinZ on Android devices. To generate an APK file, follow these instructions.

## 1. Install python-for-android (p4a)
p4a is used to package Electrum-BitcoinZ, Python, SDL and a bootstrap Java app into an APK file. 
We patched p4a to add some functionality we need for Electrum-BitcoinZ. Until those changes are
...
In the `gui/kivy` directory of Electrum-BitcoinZ, run `make theming`.

## 6. Download Electrum-BitcoinZ dependencies
